...
const toggleLike = async (req, res) => {
    ...
    try {
        ...
        const existingLike = await db.CardLike.findOne({ where: { user_id: userId, card_id: cardId } });
        ...
        if (existingLike) {
            ...
            // 좋아요 추가! 
            await db.CardLike.create({ user_id: userId, card_id: cardId });
        }
        
        // 좋아요 수 갱신 
        const likesCount = await db.CardLike.count({ where: { card_id: cardId } });
